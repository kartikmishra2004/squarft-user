  ...
  return (
    <View className="flex-1 bg-white">
      ...
      <View className="flex-row items-center pt-2 mx-4">
        {["Upcoming", "Past"].map((tab) => {
          ...
          return (
            ...
            >
              <Text
                className={`text-[15px] font-manrope-bold ${isActive ? "text-[#4A43EC]" : "text-[#9CA3AF]"
                  }`}
              >
                {tab}
              </Text>
      ...
      <ScrollView className="flex-1 bg-white" contentContainerStyle={{ paddingBottom: 120 }} showsVerticalScrollIndicator={false}>
        {activeTab === "Upcoming" && (
          <>
            <View className="mx-4 mt-5 bg-[#F8F7FF] rounded-[16px] border border-[#EBE9FF] p-[10px] flex-row items-center mb-1">
              <View className="w-[46px] h-[46px] bg-[#4A43EC] rounded-[14px] items-center justify-center mr-4">
              ...
              <View className="flex-1 justify-center">
                ...
                <Text className="text-[12px] font-manrope font-medium text-[#948FF2]">
                  See the update
                </Text>
              </View>
              <Pressable className="bg-[#6A64F1] px-5 py-[8px] rounded-xl">
                <Text className="text-white text-[11px] font-manrope-extrabold">VIEW</Text>
              </Pressable>
            </View>

            {VISITS_DATA.map((visit) => (
              <View key={visit.id} className="mx-4 mt-5 bg-white rounded-[16px] border border-gray-200 overflow-hidden shadow-sm" style={{ elevation: 1, shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 3, shadowOffset: { width: 0, height: 1 } }}>
                <View className="p-[16px]">
                  <View className="flex-row mb-4">
                    <Image
                      source={{ uri: visit.image }}
                      className="w-[84px] h-[84px] rounded-[10px] mr-4"
                      resizeMode="cover"
                    />
                    <View className="flex-1 justify-center py-1">
                      <View className="bg-[#EEECFF] self-start px-[8px] py-[4px] rounded-xl mb-[6px]">
                        <Text className="text-[#4A43EC] text-[9px] font-manrope-extrabold tracking-widest uppercase">
                          {visit.status}
                        </Text>
                      </View>
                      <Text className="text-[17px] font-manrope-extrabold text-[#111827] mb-[2px] leading-6" numberOfLines={1}>
                        {visit.title}
                      </Text>
                      <View className="flex-row items-center mt-1">
                        <Feather name="map-pin" size={12} color="#9CA3AF" />
                        <Text className="text-[#6B7280] text-[12px] font-manrope ml-1" numberOfLines={1}>
                          {visit.location}
                        </Text>
                      </View>
                    </View>
                  ...
                  <View className="mx-[-16px] mb-4" style={{ height: 1, backgroundColor: '#F3F4F6' }} />

                  <View className="mb-[14px]">
                    <Text className="text-[10px] text-[#9CA3AF] font-manrope-extrabold tracking-[1px] uppercase mb-1">
                      DATE & TIME
                    </Text>
                    <Text className="text-[14px] font-manrope-extrabold text-[#374151]">
                      {visit.dateFull}
                    </Text>
                  </View>

                  <View className="flex-row justify-between w-full">
                    <Pressable className="flex-1 bg-[#4A43EC] rounded-[12px] py-[13px] flex-row items-center justify-center mr-2">
                ...
                <View key={visit.id} className="mx-4 mt-5 bg-white rounded-[16px] border border-gray-200 overflow-hidden shadow-sm" style={{ elevation: 1, shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 3, shadowOffset: { width: 0, height: 1 } }}>
                  <View className="p-[16px]">
                    <View className="flex-row mb-4">
                      ...
                      />
                      <View className="flex-1 justify-center py-1">
                        <View className={`self-start px-[8px] py-[4px] rounded-xl mb-[6px] ${isCompleted ? 'bg-[#E5F7F1]' : 'bg-[#FEE2E2]'
                          }`}>
                    ...
                    {isCompleted ? (
                      ...
                      <View className="w-full">
                        <Pressable className="w-full bg-[#F4F2FF] rounded-[12px] py-[13px] flex-row items-center justify-center">
                          <Feather name="calendar" size={16} color="#4A43EC" />
                          <Text className="text-[#4A43EC] font-manrope-extrabold text-[14px] ml-2">
                            Re-book Site Visit
                          </Text>
