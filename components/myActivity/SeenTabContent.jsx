...
const SeenTabContent = () => {
  ...
  const SEEN_PROPERTIES = properties.filter((p) => p.isSeen);

  if (SEEN_PROPERTIES.length === 0) {
    return <EmptyState type="SEEN" />;
  }
  ...
  return (
    <ScrollView className="flex-1 bg-white" contentContainerStyle={{ paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
      ...
      <View className="mt-10 px-4 mb-6">
        {SEEN_PROPERTIES.map((property, index) => (
      ...
      <View className="px-4">
        <View className="flex-row justify-between items-center mb-3">
          <Text className="text-[15px] font-manrope-extrabold text-[#111827]">{SEEN_PROPERTIES.length} Seen Properties</Text>
